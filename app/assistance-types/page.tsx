...
import { useQueryGetProvinces, useQueryGetDistricts, useQueryGetAllDistricts } from "@/api/admin";
import { getAdminDistrictIds, isSuperAdmin } from "@/api/login";
...
function mapTypeToPayload(type: AssistanceType): AssistanceTypePayload {
  return {
    ...
    district: type.district ? { id: type.district.id } : undefined,
  };
}

export default function AssistanceTypesPage() {
  const superAdmin = isSuperAdmin();
  const adminDistrictIds = getAdminDistrictIds();
  ...
  const allDistrictsQuery = useQueryGetAllDistricts();
  const districtsForProvinceQuery = useQueryGetDistricts(selectedProvince);

  // The actual district options based on role
  const districtOptions = useMemo(() => {
    if (superAdmin) {
      return districtsForProvinceQuery.data || [];
    } else {
      const all = allDistrictsQuery.data || [];
      return all.filter(d => adminDistrictIds.includes(d.id));
    }
  }, [superAdmin, districtsForProvinceQuery.data, allDistrictsQuery.data, adminDistrictIds]);

  // Auto-select first district if none selected
  useEffect(() => {
    if (!selectedDistrictId && districtOptions.length > 0) {
      setSelectedDistrictId(districtOptions[0].id);
    } else if (selectedDistrictId && !districtOptions.find(d => d.id === selectedDistrictId)) {
      setSelectedDistrictId(districtOptions.length > 0 ? districtOptions[0].id : null);
    }
  }, [districtOptions, selectedDistrictId]);

  const assistanceTypesQuery = useQueryGetAllAssistanceTypes(selectedDistrictId);
  ...
  if (provincesQuery.isPending || allDistrictsQuery.isPending || (superAdmin && districtsForProvinceQuery.isPending)) {
    return <Loader />;
  }
  ...
  return (
    <>
      ...
      <div className="px-[3%] py-5 font-kanit">
        ...
        <div className="mb-6 rounded-xl border border-[#d9d9d9] bg-white p-4 shadow-sm">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            {superAdmin && (
              <div className="w-full md:w-1/3">
                <label className="block text-sm font-bold text-gray-700 mb-1">เลือกจังหวัด</label>
                <select
                  value={selectedProvince}
                  onChange={(e) => setSelectedProvince(e.target.value)}
                  className="w-full rounded-lg border border-[#cccccc] px-3 py-2 outline-none focus:border-[#ff3388]"
                >
                  {(provincesQuery.data || []).map(p => (
                    <option key={p.id} value={p.nameInThai}>{p.nameInThai}</option>
                  ))}
                </select>
              </div>
            )}
            ...
            <div className="w-full md:w-1/3">
              <label className="block text-sm font-bold text-gray-700 mb-1">เลือกเขตที่ต้องการจัดการ</label>
              <select
              ...
              >
                {districtOptions.map(d => (
                  <option key={d.id} value={d.id}>{d.nameInThai}</option>
                ))}
              </select>
            </div>
          </div>
            ...
            {assistanceTypesQuery.isPending ? (
              ...
              <div className="overflow-x-auto rounded-xl border border-[#d9d9d9] bg-[#EFEFEF] p-4">
                ...
                {activeTypes.length === 0 && (
                  <div className="py-6 text-center text-[2.2vmin] text-[#777777]">ยังไม่มีประเภทเรื่องในเขตนี้ (เพิ่มข้อมูลเพื่อเริ่มต้นใช้งาน)</div>
                )}
              </div>
            )}
          </>
        ) : (
          <div className="py-10 text-center text-gray-500">กรุณาเลือกเขตเพื่อจัดการประเภทเรื่องแจ้งเหตุ</div>
        )}
      </div>
    </>
  );
